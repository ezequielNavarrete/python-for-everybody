...
Actual problem: Start at: http://py4e-data.dr-chuck.net/known_by_Sofian.html
...
few seconds to make it difficult for you to do the assignment without writing a Python program. 
ut frankly with a little effort and patience you can overcome these attempts 
to make it a little harder to complete the assignment without writing a Python program. 
...
repito=int(input("repeat how many? "))
url = input('Enter - ')
...
while repito>0:
    ...
    soup = BeautifulSoup(html, 'html.parser')
    repito-=1
nombre=re.findall("_by_([A-Za-z]+).",newUrl)
print(nombre[0])